...
// reads reconcile.yml `schedules` (a list of { at: "HH:mm", scope: all|diff }) and, for each
// entry whose time is due this minute and has not yet fired today, triggers
// direct:commerce-reconcile once with that scope. Per-slot "fired today" state lives in JCR
// so a restart does not double-fire, and the whole tick is cluster-locked so only one node
// fires each slot.
//
// The hourly commerce-reconcile timer remains the baseline "all" coverage; schedules are
// additive (typically frequent "diff" passes). Times are in the server's local timezone.
...
try {
    def nowTime = java.time.LocalTime.now()

// Product Information Management endpoint (admin). Category G (#23).
//
...
// the sync endpoint (POST sync.groovy {"action":"metafields","productId":...}).
//
...
try {
    if (request.getMethod() == "GET") {
        ...
        if (raw) {
            respond(200, [productId: productId, pim: Pim.read(repositorySession, productId)])
            return
        }
    ...
    if (request.getMethod() == "POST") {
        ...
        def productId = reqMap.productId?.toString()
        if (!productId) {
        ...
        try {
            ...
            respond(200, [ok: true, productId: productId, pim: saved])
        } catch (Exception e) {
